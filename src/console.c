...
char *Con_Quakebar (int len)
{
	...
	len = fmin(len, sizeof(bar) - 2);
	len = fmin(len, con_linewidth);
...
#define MAXGAMEDIRLEN	1000
	...
	if (COM_CheckParm("-consize"))
		con_buffersize = fmax(CON_MINSIZE,Q_atoi(com_argv[COM_CheckParm("-consize")+1])*1024);
	else
...
void Con_CenterPrintf (int linewidth, char *fmt, ...)
{
	...
	linewidth = fmin (linewidth, con_linewidth);
	for (src = msg; *src; )
	{
		dst = line;
		while (*src && *src != '\n')
