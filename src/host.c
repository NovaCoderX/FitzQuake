...
void Host_InitLocal (void)
{
	...
	Cvar_RegisterVariableEx (&max_edicts, Max_Edicts_f); //johnfitz
...
void Host_Init (quakeparms_t *parms)
{
	...
	Con_DPrintf("Host_Init: memsize = %d\n", parms->memsize);
	Con_DPrintf("Host_Init: minimum_memory = %d\n", minimum_memory);

	if (parms->memsize < minimum_memory) {
		Sys_Error ("Only %4.1f megs of memory requested, can't execute game", parms->memsize / (float)0x100000);
    }

	SDL_putenv("SDL_RELATIVE_MOUSE_MODE=1");
	...
	if (cls.state != ca_dedicated)
	{
		...
		if (!host_colormap) {
            Sys_Error ("Couldn't allocate memory for the color map");
        }
	...
	// Load the config file and set the registered CVARS.
